...
export default function RelationshipCalculator() {
  ...
  const playPronunciation = () => {
    ...
    // Set the language for Cantonese
    const utterance = new SpeechSynthesisUtterance(result.cantonese);
    ...
    audioRef.current = new Audio(); // Placeholder to track state
  };
      ...
      {result ? (
        <div className="mb-3 p-3 bg-green-50 border border-green-200 rounded-md">
          <div className="flex items-center justify-between">
            <div className="text-3xl font-bold text-green-700">{result.cantonese}</div>
            <button 
              onClick={playPronunciation}
              disabled={isPlaying || !result.pinyin}
